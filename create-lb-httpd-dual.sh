...
--output text \
#--dry-run \
)) && echo "created instance ids=${EC2_IDS[@]}"

i=1
for EC2_ID in "${EC2_IDS[@]}"; do
  echo "setting for ${EC2_ID}.."
  echo "adding tags.."
  aws ec2 create-tags --resources ${EC2_ID} --tags '[{"Key": "Name", "Value": "httpd"}]'
  aws ec2 create-tags --resources ${EC2_ID} --tags "[{\"Key\": \"Number\", \"Value\": \"$i\"}]"
  EC2_PUBLIC_NAME=$(aws ec2 describe-instances --instance-ids ${EC2_ID} --query 'Reservations[].Instances[].PublicDnsName' --output text)
  ...
  ${SSH_CMD} "echo \"hello,world(#${i})\" | sudo tee /var/www/html/index.html"
  let i++
done
